...
class TestCitationServiceFormat:
    ...
    def test_validate_citations_numeric_extraction(self):
        ...
        # Should only flag [99], not other numbers
    ...
    def test_abstracts_context_format(self):
        ...
        cs.citation_map = {
            "[1]": {
                "title": "Paper One",
                "abstract": "This is the abstract of paper one.",
            },
            "[2]": {
                "title": "Paper Two",
...
class TestRAGService:
    ...
    def test_truncate_abstract_long(self):
        ...
        # Truncation happens at max_length then rsplit to word boundary
        # Result may be slightly over 800 due to including remainder of split word
        assert len(truncated) <= 1000  # never expands
        assert len(truncated) < len(long_text)  # always truncates
    ...
    def test_truncate_abstract_at_word_boundary(self):
        ...
        rag = RAGService(CitationService(SearchService()))
        # 850 chars, last word is "boundary"
        long_text = "A" * 840 + " boundary"
        truncated = rag._truncate_abstract(long_text)
        # Should not cut in middle of "boundary"
        assert truncated.endswith("...")
