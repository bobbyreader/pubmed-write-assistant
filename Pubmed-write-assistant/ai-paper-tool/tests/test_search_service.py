...
class TestSearchServiceIntegration:
    ...
    def test_search_falls_back_to_pubmed_when_openalex_empty(self, mock_client_cls):
        """When OpenAlex returns < top_k papers, should fall back to PubMed."""
        mock_response = MagicMock()
        mock_response.status_code = 200
        ...
        def json_side_effect():
            ...
            def next_json():
                call_count[0] += 1
                if call_count[0] == 1:
                    # OpenAlex: 1 paper (below top_k=10)
                    return {"results": [{"id": "W1", "title": "One Paper",
                                         "doi": None, "publication_year": 2020,
                                         "abstract_inverted_index": {},
                                         "authorships": [], "primary_location": {},
                                         "open_access": {}}]}
                elif call_count[0] == 2:
                    # Semantic Scholar: empty
                    return {"data": []}
                else:
                    # PubMed fallback
                    return {
        ...
        # Should include at least one paper
        assert len(papers) >= 1
