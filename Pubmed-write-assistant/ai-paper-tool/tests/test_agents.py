...
class TestWriterAgentParse:
    ...
    def test_writer_parse_truncated_json_single_close(self):
        """Truncated at end of string value — try adding } to close."""
        from agents.writer import WriterAgent
...
class TestReviewerAgentParse:
    ...
    def test_reviewer_repair_skips_truncation_artifacts(self):
        ...
        agent = object.__new__(ReviewerAgent)
        result = agent._try_repair_json(raw)
        # Should skip the unterminated string line
        assert isinstance(result, dict)

    def test_reviewer_parse_strips_json_lang(self):
        """reviewer uses re.sub for ```json? which matches ```json or ```js, not bare ```"""
        from agents.reviewer import ReviewerAgent

        # Bare ``` without lang tag — re.sub won't strip it, JSON parse fails, defaults returned
        raw = """```
        {"summary": "No lang", "score": 8, "citation_accuracy_score": 9, "format_compliance_score": 8, "strengths": [], "weaknesses": [], "hallucination_flags": [], "suggestions": []}
        ```"""
        agent = object.__new__(ReviewerAgent)
        data = agent.parse_response(raw)
        # Bare ``` is not stripped by re.sub pattern, parse fails → defaults
        assert data["score"] == 5  # default, not 8
...
class TestEditorAgentParse:
    ...
    def test_editor_parse_truncated_unclosed_string_returns_none(self):
        """Unclosed string value: even all suffixes can't fix it → returns None."""
        from agents.editor import EditorAgent

        raw = '{"revised_draft": "Starts here'
        agent = object.__new__(EditorAgent)
        result = agent._try_repair_json(raw)
        # Unclosed string: "Starts here` + "}" → `{"revised_draft": "Starts here}` still invalid
        # None of the suffixes can close the unclosed string
    ...
    def test_editor_parse_unrecoverable(self):
        ...
        agent = object.__new__(EditorAgent)
        result = agent._try_repair_json(raw)
        # All suffixes fail — returns None
        assert result is None
